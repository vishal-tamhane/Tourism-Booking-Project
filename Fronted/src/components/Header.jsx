...
function Header({ showSearch = true, onSearch }) {
  ...
  return (
    <div className="bg-white border-b-4 border-primary mb-6 md:mb-8">
      <div className="flex flex-col md:flex-row items-center justify-between px-4 md:px-10 py-4 md:py-5 gap-4">
        <button
          onClick={() => navigate('/')}
          className="flex items-center gap-0 cursor-pointer text-black bg-transparent border-0 p-0"
        ...
        >
          ...
          <span className="text-lg md:text-xl font-bold leading-tight lowercase tracking-tight">
            Highway<br/>Delite
          </span>
        </button>
        ...
        {showSearch && (
          <div className="flex items-center gap-2 w-full md:flex-1 md:max-w-2xl md:mx-10">
            <input 
              type="text" 
              placeholder="Search destinations (e.g., Paragliding, Kayaking...)" 
              value={searchQuery}
